...
class BlackJack:
    ...
    async def blackjack(self, ctx, *, bet: int = 0):
        ...
        if bet < 10:
            await ctx.send("{}\n".format(ctx.author.mention) + "You can't possibly be that cheap dude.")
            return
        ...
        if await koyomi_user.get_coins() >= bet >= 10:
            ...
            await ctx.send("{}\n".format(ctx.author.mention) + "You don't have enough money.")
            return
        ...
        for _ in range(2):
            player_hand.append(newdeck.deal_card())
            dealer_hand.append(newdeck.deal_card())
        ...
        if player_score == 21:
            if dealer_score == 21:
                await ctx.send("{}\n".format(ctx.author.mention) + "Both players got blackjack. Got your money back!")
            ...
            await ctx.send("{}\n".format(ctx.author.mention) + "Dealer got blackjack. You lose.")
            return
        else:
            await ctx.send(
                "{}".format(ctx.author.mention)
    ...
    async def jhit(self, ctx):
        if ctx.author.id not in self.players:
            ...
            return
        self.players[ctx.author.id]["player_hand"].append(self.players[ctx.author.id]["deck"].deal_card())
        ...
        if player_score > 21:
            ...
            await ctx.send("{}\n".format(ctx.author.mention) + "You HIT. You have {}.\n".format(", ".join(output)))
    ...
    async def jstand(self, ctx):
        ...
        while self.get_score(self.players[ctx.author.id]["dealer_hand"]) < 17:
            ...
            self.players[ctx.author.id]["dealer_hand"].append(card)
        ...
        del self.players[ctx.author.id]
        await ctx.send("{}\n".format(ctx.author.mention) + " ".join(output))
    ...
    async def jsurrender(self, ctx):
        if ctx.author.id in self.players:
            ...
            await koyomi_user.set_coins(await koyomi_user.get_coins() + (self.players[ctx.author.id]["bet"] // 2))
            del self.players[ctx.author.id]
        else:
            await ctx.send("No BlackJack Game in Session")
