...
class Tags:
    def __init__(self, bot):
        self.bot = bot
        if not self.bot.loop.run_until_complete(redis_pool.hexists("tags", "lol")):
            self.bot.loop.run_until_complete(redis_pool.hmset_dict("tags", {"lol": "xD"}))
    ...
    async def add(self, ctx, *, tag: str):
        ...
        if await redis_pool.hexists("tags", tag):
            ...
            value = await self.bot.wait_for("message", check=check, timeout=15)
            await asdf.delete()
            if value is None:
