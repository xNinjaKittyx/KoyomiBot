...
class Slots:
    def __init__(self, bot):
        self.bot = bot
        self.emojis = [":100:", ":snake:", ":watermelon:", ":cherries:"]
        if not self.bot.loop.run_until_complete(redis_pool.get("jackpot")):
            self.bot.loop.run_until_complete(redis_pool.set("jackpot", 0))
    ...
    async def slots(self, ctx, bet: int):
        ...
        if await user.get_coins() >= bet >= 0:
            ...
            final += f"\nPlaced {bet} into the machine\n"
            if len(result) == 3:
                final += "\nBetter Luck Next Time."
                await redis_pool.incrby("jackpot", max(int(bet * 0.5), 1))
